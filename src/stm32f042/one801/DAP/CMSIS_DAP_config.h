...
/* No JTAG on this board, so JTDI and JTDO are deliberately left undefined.
 *
 * JTDO was PB7, which is USART1_RX here -- the Wi-Fi module's UART. The HAL
 * claims that pin whenever JTDO is *defined*, not only when JTAG is selected:
 * PORT_SWD_SETUP and PORT_OFF both reconfigure it to a plain input, which
 * would drop the pin's alternate function and silently kill the module's
 * receive path. Defining it and never selecting JTAG would not be enough.
 *
 * The STM32H5 target is a Cortex-M33 debugged over SWD, so nothing is lost.
 */
