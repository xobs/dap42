...
void gpio_setup(void) {
    ...
    /* Wi-Fi module: powered and set to boot normally.
     *
     * Set here rather than left to the host, because USB modem-control lines
     * start deasserted, and deasserted DTR means download mode. A board that
     * waited for the host would come up with the module sitting in the
     * bootloader whenever nothing had opened the port.
     */
    rcc_periph_clock_enable(WIFI_GPIO_CLOCK);
    gpio_mode_setup(WIFI_GPIO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE,
                    WIFI_EN_PIN | WIFI_BOOT_PIN);
    gpio_set(WIFI_GPIO_PORT, WIFI_EN_PIN);
    gpio_clear(WIFI_GPIO_PORT, WIFI_BOOT_PIN);
}

void target_set_control_lines(bool dtr, bool rts) {
    /* Inverted, matching a TTL USB-serial adapter, so that host tooling
     * written against one works here unchanged. See config.h for the table. */
    if (rts) {
        gpio_clear(WIFI_GPIO_PORT, WIFI_EN_PIN);
    } else {
        gpio_set(WIFI_GPIO_PORT, WIFI_EN_PIN);
    }
    ...
    if (dtr) {
        ...
        gpio_set(WIFI_GPIO_PORT, WIFI_BOOT_PIN);
    }
}
